...
// Comprehensive system for template likes, bookmarks, reviews, and sharing
...
export function useTemplateLikes(templateId: string) {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  
  // Get like status and count
  const { data: likeData, isLoading } = useQuery(
    ['template-likes', templateId],
    async () => {
  ...
  const toggleLikeMutation = useMutation(
    ...
    },
    {
      onSuccess: (data) => {
        ...
        queryClient.setQueryData(['template-likes', templateId], (old: any) => ({
          ...old,
...
export function useTemplateBookmarks(templateId: string) {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  
  const { data: bookmarkData, isLoading } = useQuery(
    ['template-bookmarks', templateId],
    async () => {
  ...
  const toggleBookmarkMutation = useMutation(
    ...
    {
      onSuccess: (data) => {
        queryClient.setQueryData(['template-bookmarks', templateId], (old: any) => ({
          ...old,
...
export function useTemplateReviews(templateId: string) {
  ...
  const submitReviewMutation = useMutation(
    ...
    {
      onSuccess: () => {
        queryClient.invalidateQueries(['template-reviews', templateId]);
        queryClient.invalidateQueries(['template', templateId]);
...
export function useTemplateSharing(templateId: string) {
  ...
  const trackShareMutation = useMutation(
    async (platform: string) => {
      ...
      // Update share count
      await supabase.rpc('increment_template_shares', {
        template_id: templateId
      });
    }
  );
...
export function useSocialStats(templateId: string): SocialStats {
  const { likeCount, isLiked } = useTemplateLikes(templateId);
  const { bookmarkCount, isBookmarked } = useTemplateBookmarks(templateId);
  const { reviews } = useTemplateReviews(templateId);
  
  const { data: templateData } = useQuery(
    ['template-social', templateId],
    async () => {
      const { data, error } = await supabase
        .from('templates')
