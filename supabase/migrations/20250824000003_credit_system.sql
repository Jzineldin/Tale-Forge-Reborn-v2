...
('story_image', 2, 'Cost per story illustration'),
('story_audio', 2, 'Cost per audio narration segment')
...
-- Function to refresh monthly credits for free tier users
CREATE OR REPLACE FUNCTION refresh_monthly_credits(user_uuid UUID)
RETURNS BOOLEAN AS $$
DECLARE
  user_role TEXT;
...
-- Trigger to automatically initialize credits when user profile is created
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'on_user_profile_created_init_credits') THEN
    CREATE TRIGGER on_user_profile_created_init_credits
