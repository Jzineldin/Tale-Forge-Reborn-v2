    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorText.includes('Missing required environment variables') || 
            ...
            response.status === 500) {
          console.log('🎭 Falling back to Mock AI Service due to missing API keys');
  ...
  async generateStorySegment(userId: string, storyId: string, choiceIndex?: number): Promise<StorySegment> {
    try {
      ...
      if (!response.ok) {
        ...
        // Fall back to mock service if API keys are missing
        if (errorText.includes('Missing required environment variables') || 
            errorText.includes('API key') || 
            response.status === 500) {
          console.log('🎭 Falling back to Mock AI Service for segment generation');
