...
const AdminContentPage: React.FC = () => {
    ...
    const fetchContent = async () => {
      try {
        ...
        const transformedContent: ContentItem[] = (stories || []).map((story: any) => {
          ...
          const authorName = profile?.full_name || profile?.email?.split('@')[0] || `User ${story.user_id?.substring(0, 8) || 'Unknown'}`;
          ...
          return {
            ...
            type: story.is_completed ? 'completed_story' : 'chapter',
            createdAt: new Date(story.created_at).toLocaleDateString(),
            reports: reportCount,
            reportReasons,
  ...
  const filteredContent = content
    .filter(item => {
      ...
      const matchesFilter = filter === 'all' || 
        ...
        (filter === 'completed_stories' && item.type === 'completed_story') ||
        (filter === 'chapters' && item.type === 'chapter') ||
  ...
  const handleBulkAction = async (action: 'approve' | 'remove') => {
    try {
      console.log(`🔧 Bulk ${action} action for ${selectedItems.length} items:`, selectedItems);
      
      const confirmAction = window.confirm(
        `Are you sure you want to ${action} ${selectedItems.length} selected items?`
      );
      ...
      for (const itemId of selectedItems) {
        await handleContentAction(itemId, action);
      }
      
      setSelectedItems([]);
      console.log(`✅ Bulk ${action} action completed for ${selectedItems.length} items`);
    } catch (error) {
        ...
        <div className="glass-enhanced backdrop-blur-lg bg-black/20 border border-white/20 rounded-2xl p-8 mb-8">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-4 mt-3 text-sm">
                <span className="text-white/70">
                  Total: <span className="text-amber-400 font-bold">{filteredContent.length}</span> items
                </span>
                <span className="text-green-400">
                  📖 <span className="font-bold">{content.filter(item => item.type === 'completed_story').length}</span> Complete Stories
                </span>
                <span className="text-blue-400">
                  📝 <span className="font-bold">{content.filter(item => item.type === 'chapter').length}</span> Chapters
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            >
              ...
              <option value="completed_stories">📖 Complete Stories</option>
              <option value="chapters">📝 Chapters (In Progress)</option>
                        ...
                        <div>
                          ...
                          <div className="text-white/60 text-sm">
                            <span className={`px-2 py-1 rounded text-xs font-medium mr-2 ${
                              item.type === 'completed_story' ? 'bg-green-500/20 text-green-400' : 
                              item.type === 'chapter' ? 'bg-blue-500/20 text-blue-400' : 
                              'bg-gray-500/20 text-gray-400'
                            }`}>
                              {item.type === 'completed_story' ? '📖 Complete Story' : 
                               item.type === 'chapter' ? '📝 Chapter' : 
                               item.type}
                            </span>
                            {item.genre} • Ages {item.ageGroup}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
